...
func (s *server) heartbeatChecker() {
	...
	for range ticker.C {
		...
		for _, group := range s.consumerGroups {
			...
			// Without this check, the heartbeatChecker would see the lack of heartbeat,
			...
			for consumerID, member := range group.Members {
				if time.Since(member.LastHeartbeat) > sessionTimeout {
					log.Printf("consumer %s in group %s time out. triggering rebalance.", consumerID, group.GroupID)
...
func (s *server) startRebalance(group *ConsumerGroup) {
	...
	log.Printf("group %s state changed to rebalance in progress", group.GroupID)
...
func (s *server) finishJoinPhase(group *ConsumerGroup) {
	...
	// Handle the case when a rebalance ends with an empty group.
...
func (s *server) finishSyncPhase(group *ConsumerGroup) {
	log.Printf("fishing sync phase for group %s generation %d", group.GroupID, group.generationID)
...
func (s *server) SyncGroup(ctx context.Context, req *api.SyncGroupRequest) (*api.SyncGroupResponse, error) {
	...
	if len(req.Assignments) > 0 {
		// This is the leader's sync request
	...
	// If all member have now synced, finish the rebalance.
...
func (s *server) Heartbeat(ctx context.Context, req *api.HeartbeatRequest) (*api.HeartbeatResponse, error) {
	...
	log.Printf("heartbeat for consumer %s: group %s state: %s", req.ConsumerId, req.GroupId, group.State)

	member, ok := group.Members[req.ConsumerId]
	if !ok {
		return &api.HeartbeatResponse{ErrorCode: api.ErrorCode_UNKNOWN_MEMBER_ID}, nil
	}
